...
def test_dates_start_invalid_is_rejected(asyncio_event_loop, monkeypatch):
    agent = DummyAgent()
    beh = DummyBehaviour()
    msg = DummyMsg()

    bad_fact = AclMessage.build_inform_fact("conv-bad-d", "dates_start", "2026-13-40", ontology="travel")
...
def test_dates_start_valid_is_confirmed(asyncio_event_loop, monkeypatch):
    agent = DummyAgent()
    beh = DummyBehaviour()
    msg = DummyMsg()

    good_fact = AclMessage.build_inform_fact("conv-ok-d", "dates_start", "2026-02-10", ontology="travel")
    ...
    assert calls and calls[0][0] == "conv-ok-d"
    assert calls[0][1] == "dates_start"
    assert calls[0][2]["value"] == "2026-02-10"
